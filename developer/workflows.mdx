...
The `create_workflow` method requires a `name` for the workflow and a list of `tasks`. Each item in the `tasks` list defines a step and **must contain either** an `instructions` string (with an optional `target_url`) **or** the `task_id` of a pre-defined task (like one generated from video). You cannot provide both for the same step.
