...
This guide explains how to use the Python SDK to upload a video for automatic task/workflow generation and check the processing status.
...
This feature allows you to generate a task or workflow definition by providing a video recording of the desired actions. The processing happens asynchronously.

## Uploading Video for Task/Workflow Generation

Upload a video file and associate it with an end-user to start the generation process.
...
try:
    ...
    job_details = agent.create_task_from_video( # Updated method name
        ...
        workflow_name=optional_task_name # Optional: Name for the generated task/workflow
    )
...
The `get_task_processing_job_status` method requires the `job_id`. It returns a dictionary containing the job's current `status`. If completed, it includes the `workflow_id` of the generated workflow definition (based on current API spec). If failed, it includes an `error` message. It may also contain timestamps. Returns `None` on error.
